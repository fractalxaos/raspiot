...
<?php
/*
 Script: altimeter.php

 Description: This scripts generates on the server charts showing
 altimeter data spanning the period supplied by the user.  The script
 does the following:
...
define("_RRD_FILE", str_replace("public_html/altimeter/altimeter.php",
                                "database/altimeterData.rrd",
                                $_SERVER["SCRIPT_FILENAME"]));
# charts html directory
define("_CHART_DIRECTORY", str_replace("altimeter.php",
                                       "dynamic/",
                                       $_SERVER["SCRIPT_FILENAME"]));
...
if ($beginDateEp > $endDateEp) {
    ...
    createChart('custom_altitude', 'altitude', 'meters', 
                'Altitude', $beginDateEp, $endDateEp,
                 0, 0, 0, true);
    createChart('custom_pressure', 'pressure', 'inches\ Hg', 
                'Barometric\ Pressure', $beginDateEp, $endDateEp,
                 0, 0, 0, true);
    createChart('custom_temperature', 'temperature', 'degrees\ Fahrenheit', 
                'Temperature', $beginDateEp, $endDateEp,
                 0, 0, 0, true);
    # Send html commands to client browser.
    echo "<div class=\"chartContainer\">" .
         "<img class=\"chart\" src=\"dynamic/custom_altitude.png\">" .
         "</div>";
    echo "<div class=\"chartContainer\">" .
         "<img class=\"chart\" src=\"dynamic/custom_pressure.png\">" .
         "</div>";
    echo "<div class=\"chartContainer\">" .
         "<img class=\"chart\" src=\"dynamic/custom_temperature.png\">" .
         "</div>";
}
...
function createChart($chartFile, $dataItem, $label, $title, $begin,
    ...
    */
    ...
    $cmdfmt = "DEF:dSeries=%s:%s:AVERAGE ";
