...
class rrdbase:
    ...
    def updateDatabase(self, tData):
        ...
        # Note that this is the tuple remaining after the
        # first item (the date) has been removed by the above code.
        tData = tuple(ltData)
        strFmt = 'rrdtool update %s %s'
        for item in tData:
            strFmt += ':%s'
        # Python3 expands tData into individual arguments.
        strCmd = strFmt % (self.rrdFile, time, *tData)
        ...
        try:
            ...
            print('%s rrdtool update failed: %s' % \
                  (rrdbase.getTimeStamp(), exError.output))
            return False
